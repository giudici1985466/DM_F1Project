-- ROLL-UP: total points for each team in each continent
...
-- SLICING: mean points for each pilot in "wet" races
...
-- SLICING/DICING/ROLL-UP: top 5 teams in high altitude circuits
WITH circuit_races AS (
...
-- ROLL-UP/PIVOTING teams comparison across continents (teams on rows, continents on columns)
...
-- DRILL-DOWN/RANKING: best race for each team
...
-- PIVOTING: Team scores for each decade
SELECT 
    c.name AS team,
...
-- DICING/PIVOTING:comparison between the nationalities of the drivers and the points scored on the continents
...
--SLICING constructor,circuit,team podium tuple with same nationality
...
--DICING/ROLL-UP drivers ranking for points achieved in hot and humid races in 2023 and 2024 seasons
...
--RANKING drivers ranking on position delta, only including drivers with at least 50 completed races
...
--RANKING circuit ranking by average number of overtakes, only including circuits that hosted at least 10 races
SELECT 
    c.circuit_id,
